...
FROM node:boron
```
#### # 拷贝index.js 到当前的目录 (这里还可以使用 copy . .)
```
COPY index.js .
...
## docker build
docker 执行默认是以超级管理员用户执行的 如果没有的话就会是

`Got permission denied while trying to connect to the Docker daemo` 这样的错误
...
这条命令 就会把 容器内3000 端口绑定到当前的80端口上
